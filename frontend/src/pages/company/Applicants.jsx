...
export default function Applicants() {
  ...
  const handleAIShortlist = async () => {
    ...
    try {
      ...
      fetchApplicants(); // Reload with scores
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">{applications.length} applications received</p>
          </div>
            ...
            {applications.map((app, idx) => (
              <div key={app._id} className="card">
                ...
                {interviewForm === app._id && (
                  <form onSubmit={handleScheduleInterview} className="mt-4 p-4 bg-primary-50 rounded-lg space-y-3">
                    <h4 className="font-medium text-sm text-gray-800">Schedule Interview</h4>
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="text-xs text-gray-600 mb-1 block">Meet Link (if online)</label>
                        <input name="meetLink" type="url" className="input text-xs" placeholder="https://meet.google.com/..." />
                      </div>
                      <div>
                        <label className="text-xs text-gray-600 mb-1 block">Venue (if offline)</label>
